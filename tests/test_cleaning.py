...
from src.cleaning.cleaning import to_lowercase, drop_street_numbers, clean_abbreviation
from src.cleaning.cleaning import fusion_addresses_columns, clean_hebergement_with_only_avec_sans
...
@pytest.mark.cleaning
def test_fusion_addresses_columns():
    # Spark Context initialisation
    spark_context = SparkContext()
    sql_context = SQLContext(spark_context)
    input_df = sql_context.createDataFrame(
    ...
    real_df = fusion_addresses_columns(input_df, "address_1", "address_2")
    real_df = get_sorted_data_frame(
        real_df.toPandas(),
        ["row_number","address_1"],
    )
...
def test_cleaning_addresses_with_geometrie():
    ...
    sql_context = SQLContext(spark_context)
    input_df = sql_context.createDataFrame(
        [("44.88067,-0.570246", "Loge N° 41", "C.C.AUCHAN LAC : MINELLI"),
        ("44.88067,-0.570246", "Loge N° 147", "C.C.AUCHAN LAC: FNAC"),
        ("44.88067,-0.570246", None, "C.C.AUCHAN LAC: TRESOR"),
        ("44.88067,-0.570242","Loge N°134", "C.C.AUCHAN LAC: FRANCK PROVOST"),
    ...
    expected_df = sql_context.createDataFrame(
        [("44.88067,-0.570246", "rue de la paix", "C.C.AUCHAN LAC : MINELLI"),
    ...
    real_df = cleaning_addresses_with_geometrie(input_df, "address_1", "44.88067,-0.570246", "rue de la paix")
    real_df = get_sorted_data_frame(
        real_df.toPandas(),
        ["geometrie", "address_1", "name"],
    )
