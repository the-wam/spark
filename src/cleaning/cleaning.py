...
from pyspark.sql.functions import lower, col, regexp_replace, concat, when, lit
...
def cleaning_addresses_with_geometrie(df, column, geometrie, new_adress):
    ...
    return df.withColumn(column, when(col("geometrie") == geometrie, new_adress).otherwise(col(column)))          
...
def list_cleaning_addresses_with_geometrie(df):
    ...
    """
    df = cleaning_addresses_with_geometrie(df, "adresse_1", "44.88067,-0.570246", "avenue des quarante journaux")
    df = cleaning_addresses_with_geometrie(df, "adresse_2", "44.88067,-0.570246", lit(None).cast(StringType()))
    df = cleaning_addresses_with_geometrie(df, "adresse_1", "44.872645,-0.571176", "avenue jean gabriel domergue")
    df = cleaning_addresses_with_geometrie(df, "adresse_1", "44.839286,-0.576552", "rue pere louis de jabrun")
...
def fusion_addresses_columns(df, column1 = "adresse_1", column2 = "adresse_2"):
    """
    Create a new row with the address from the column adresse_2
    and return a DataFrame
    """
    df_address_2_not_null = df.where(col(column2).isNotNull()) \
                            .drop(column1) \
                            .withColumnRenamed(column2, column1)
                            

    return df.drop(column2) \
            .union(df_address_2_not_null)
...
def clean_abbreviation(df, column = "adresse_1"):
    ...
    return df.withColumn(column, \
              when(col(column).rlike(" ave "), regexp_replace(col(column), " ave ", " avenue ")) \
...
def drop_duplicate(df, columns_list = ["geometrie", "nb_visiteurs_max"]):
    """
...
def drop_non_adress_rows(df, column):
...
def drop_street_numbers(df, column = "adresse_1"):
    ...
    and return a DataFrame
    """
    return df.withColumn(column, regexp_replace(col(column), r'(^ *)\d* ?/?-?à? ?\d* ?,? *(bis)?(ter)?b?t?,? *', ""))
...
def cleainng_order(df):
    ...
    if check_columns(df):
        ...
        df = fusion_addresses_columns(df)
        df = drop_duplicate(df)
        df = to_lowercase(df)
        df = split_address(df) # to do 
